...
class TrackingNode(Node):
    ...
    def detected_obs_pose_callback(self, msg):
        ...
        #----------This line is commented out in original code-----------
        ...
        if np.linalg.norm(center_points) > 3 or center_points[2] > 0.7:
            self.obs_pose = None
            self.get_logger().info('Detected Object Filtered')
            return
        #-----------------------------------------------------------
    ...
    def detected_goal_pose_callback(self, msg):
        ...
        if np.linalg.norm(center_points) > 3 or center_points[2] > 0.7:
            self.goal_pose = None
            self.get_logger().info('Detected Goal Filtered')
            return
        #-------------------------------------------------------        
    ...
    def get_current_poses(self):
        ...
        #----------NEW (to handle the None obstacle pose properly)---------
        obstacle_pose = None
    ...
    def timer_update(self):
        ...
        # Switch target to home when robot within acceptable distance (0.3) to goal
        goal_dist  = np.linalg.norm(current_goal_pose[:2])
        if goal_dist < 0.3:
    ...
    def controller(self, goal_pose, obs_pose=None):
